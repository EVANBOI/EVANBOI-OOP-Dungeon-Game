...
public class PotionsTest {
    ...
    public void invincibilityPotionZombieMovement() throws InvalidActionException {
        ...
        double prevMagnitude = TestUtils.getEuclideanDistance(playerPos, zombiePos);

        for (int i = 0; i < 10; i++) {
            res = dmc.tick(Direction.DOWN);
            zombiePos = TestUtils.getEntities(res, "zombie").get(0).getPosition();
            double currMagnitude = TestUtils.getEuclideanDistance(playerPos, zombiePos);
            assertTrue(currMagnitude > prevMagnitude);
    ...
    public void invincibilityPotionMercenaryMovement() throws InvalidActionException {
        ...
        double prevMagnitude = TestUtils.getEuclideanDistance(playerPos, mercPos);

        for (int i = 0; i < 10; i++) {
            res = dmc.tick(Direction.DOWN);
            mercPos = TestUtils.getEntities(res, "mercenary").get(0).getPosition();
            double currMagnitude = TestUtils.getEuclideanDistance(playerPos, mercPos);
            assertTrue(currMagnitude > prevMagnitude);
            prevMagnitude = currMagnitude;
        }
    }
